...
<style>
  ...
  <div id="tab-simulator" class="tab-content active">
    ...
    <p style="text-align:center; color:#ef4444; font-size:0.95rem; font-weight:bold; margin-top:-10px;">⚠️ 주의: 좌측 모바일 앱의 각 작업자 카드 테두리 안에서 마우스 휠을 스크롤하지 않으면 "움직임 없음"으로 간주되어 즉시 쓰러짐 경고가 발생합니다!</p>
  </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  [worker1, worker2].forEach(w => {
    const resetWheel = () => {
      ...
      w.wheelTimer = setTimeout(() => {
        if(w.state !== 'EMERGENCY' && w.hOn && w.vOn) {
          w.state = 'EMERGENCY';
          w.tText.style.display = 'block';
      ...
      }, 5000); // 5 seconds of no wheel -> fall
    };
    
    w.card.addEventListener('wheel', resetWheel, {passive: true});
    w.wrapper.addEventListener('wheel', resetWheel, {passive: true});
    w.card.addEventListener('mousemove', resetWheel);
    w.wrapper.addEventListener('mousemove', resetWheel);
