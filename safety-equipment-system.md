...
<style>
  ...
  /* Worker Tabs Toggle */
  .worker-tabs {
    display: flex; gap: 5px; margin-bottom: 15px; background: rgba(0,0,0,0.3); padding: 4px; border-radius: 8px;
  }
  .w-tab {
    flex: 1; background: transparent; color: #94a3b8; border: none; padding: 8px; border-radius: 6px;
    font-weight: bold; cursor: pointer; transition: all 0.2s; font-size: 0.9rem;
  }
  .w-tab.active { background: #334155; color: white; box-shadow: 0 2px 5px rgba(0,0,0,0.3); }
  .worker-status-card { display: none; }
  .worker-status-card.active { display: block; }
      ...
      <div class="sim-app">
        <div class="sim-app-ui" id="app-ui-scroll">
          ...
          <div class="worker-tabs">
            <button class="w-tab active" id="tab-w1">작업자 1</button>
            <button class="w-tab" id="tab-w2">작업자 2</button>
          </div>
          
          <div class="worker-status-card active" id="card-1">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function triggerEmergency(w) {
    ...
    updateGlobalState();
    switchTab(w.id); // Auto switch to fallen worker
  }
  ...
  function switchTab(workerId) {
    if(!tabW1) return;
    if(workerId === 1) {
      tabW1.classList.add('active'); tabW2.classList.remove('active');
      card1.classList.add('active'); card2.classList.remove('active');
    } else {
      tabW2.classList.add('active'); tabW1.classList.remove('active');
      card2.classList.add('active'); card1.classList.remove('active');
    }
  }
  
  if(tabW1) tabW1.addEventListener('click', () => switchTab(1));
  if(tabW2) tabW2.addEventListener('click', () => switchTab(2));
