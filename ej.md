...
<style>
  ...
  .sub-nav li.toc-h2 a { font-weight: 600; color: #cbd5e1; }
  .sub-nav li.toc-h3 a { padding-left: 20px; font-size: 0.85rem; }
  ...
  .gitbook-content-area figure.highlight {
    margin: 20px 0;
    border-radius: 12px;
    overflow: hidden;
...
<div class="gitbook-container">
  ...
  <aside class="gitbook-sidebar">
    <div class="gitbook-search-wrapper">
      ...
      <input type="text" id="gitbook-search" placeholder="Type to search">
    </div>
    ...
    <ul class="sidebar-nav">
      ...
      <li class="nav-category active" id="cat-python">
        <div class="nav-category-title" onclick="openCategory('python')">
          파이썬 <i class="uil uil-angle-down"></i>
        </div>
        <ul class="sub-nav" id="python-toc">
          <!-- Python TOC injected by JS -->
        </ul>
      </li>
      <li class="nav-category" id="cat-csharp">
        <div class="nav-category-title" onclick="openCategory('csharp')">C#</div>
      </li>
    </ul>
  </aside>
...
function openCategory(catId) {
  ...
  // Show target content
  document.getElementById('content-' + catId).classList.add('active');
  // Activate target category (if it's not search results)
  if(catId !== 'search-results' && document.getElementById('cat-' + catId)) {
    document.getElementById('cat-' + catId).classList.add('active');
  }
}

// Python Dynamic TOC & Full-Text Search
document.addEventListener("DOMContentLoaded", function() {
  const pyContent = document.getElementById("content-python");
  const pyToc = document.getElementById("python-toc");
  ...
  // 1. Chunk Python content into searchable sections
  let sections = [];
  let currentSection = null;
  
  // We iterate over the direct children of pyContent to group them under h2/h3
  Array.from(pyContent.children).forEach((el, index) => {
    if (el.tagName === 'H2' || el.tagName === 'H3') {
      if (currentSection) sections.push(currentSection);
      
      const id = "py-sec-" + index;
      el.id = id;
      ...
      li.addEventListener('click', (e) => {
        // If clicking TOC while in search results, go to python tab
        openCategory('python');
      });
      pyToc.appendChild(li);
      currentSection.tocElement = li;
      
    } else if (currentSection) {
      currentSection.nodes.push(el);
      currentSection.fullText += " " + el.innerText.toLowerCase();
      currentSection.rawText += " " + el.innerText;
    }
  });
  if (currentSection) sections.push(currentSection);
  ...
  searchInput.addEventListener("input", function() {
    ...
    // Filter sections based on FULL TEXT
    sections.forEach(sec => {
      if (sec.fullText.includes(query)) {
        matchCount++;
        // Show TOC link
        sec.tocElement.classList.remove("hidden-section");
        ...
        title.innerHTML = sec.titleText;
        title.addEventListener('click', () => {
          // Go to python tab and jump
          openCategory('python');
          window.location.hash = sec.id;
          // Trigger a small scroll offset if needed
          setTimeout(() => {
            sec.header.scrollIntoView({behavior: 'smooth', block: 'start'});
          }, 50);
        });
        ...
        // No match. Hide TOC link.
        sec.tocElement.classList.add("hidden-section");
      }
    });
